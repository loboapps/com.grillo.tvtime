...
drop function if exists tvtime_add_show(integer, text, text, text, text, integer, integer, text, jsonb, jsonb, date);
...
begin
  insert into tvtime_shows (
    tvmaze_id, name, poster_path, backdrop_path, tvmaze_status, imdb_id,
    number_of_seasons, number_of_episodes, user_status, next_air_date, synced_at
  )
  values (
    p_tvmaze_id, p_name, p_poster_path, p_backdrop_path, p_tvmaze_status, p_imdb_id,
...
grant execute on function tvtime_add_show(integer, text, text, text, text, text, integer, integer, text, jsonb, jsonb, date) to authenticated, service_role;
revoke execute on function tvtime_add_show(integer, text, text, text, text, text, integer, integer, text, jsonb, jsonb, date) from public, anon;
