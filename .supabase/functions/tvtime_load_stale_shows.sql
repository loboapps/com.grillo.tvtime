...
as $$
  ...
  -- finished show can still get a real revival season, announced well in advance per TVmaze's
  -- episode air dates). This is normally a tiny result set on any given day, not the whole
  -- library - the whole-library "is there a season TVmaze hasn't told us about yet" sweep is a
  ...
  select coalesce(jsonb_agg(tvmaze_id), '[]'::jsonb)
  from tvtime_shows
